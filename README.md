...
| United States | CBS East HD and USA Network | [CBS Schedule](https://www.cbs.com/schedule/) and [USA Network schedule](https://www.usanetwork.com/schedule) | Full rolling seven-day schedules for the public CBS East HD and USA-East feeds only | English |
...
CBS’s official Schedule page anonymously loads an EPG JSON whose own channel metadata identifies the feed as **CBS East HD**. The feed includes CBS New York local entries, but its official `CBS East HD` name is preserved as the sole XMLTV display name; it is not relabelled as a generic ID or as a station not named by the source. [11]
...
The project does not create empty channels when an official page identifies a service but does not offer reusable public channel-level programme data. The CBS official feed is now available, but the New York ABC/NBC/FOX stations, NBC Sports NOW, ESPN/ESPN2/ESPNews/ESPNU, FS1/FS2, TBS/truTV, France 2–5, Poland Eurosport 1–4, Eurosport 4K, and NHK domestic channels are not presently published. NBC, ABC and ESPN expose some anonymous live-guide information, but a stable reusable full channel-level schedule has not yet been confirmed. In particular, NHK’s official text guide limits its programme data to private use unless permission is obtained; no public XMLTV redistribution is made from it.
...
epg search "CBS News New York" --provider cbs_us
```
...
[11]: https://www.cbs.com/schedule/ "CBS — Schedule"
