...
def enrich_tvguide_uk_images(
    ...
    *,
    ...
    def fetch_page(key: tuple[str, str]) -> tuple[tuple[str, str], list[TVGuideScheduleEntry], bool]:
        slug, local_date = key
        try:
            response = request_session.get(
                f"{TVGUIDE_BASE_URL}/channel/{slug}", params={"date": local_date}, timeout=(3, 10)
            )
            response.raise_for_status()
            entries = parse_tvguide_schedule(response.text)
            failed = False
        ...
        return key, entries, failed

    # A small bounded pool avoids making a seven-day refresh unnecessarily slow while
    # keeping each channel/day request independent and fail-closed.
    max_workers = min(8, max(1, len(requested)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(fetch_page, key) for key in sorted(requested)]
        for future in as_completed(futures):
            key, entries, failed = future.result()
